...
namespace osu.Server.Spectator.Tests
{
    public class ScoreUploaderTests
    {
        ...
        public ScoreUploaderTests()
        {
            ...
            mockDatabase.Setup(db => db.GetScoreFromToken(1)).Returns(ValueTask.FromResult<SoloScore?>(new SoloScore
            {
                id = 2,
                passed = true
            }));
        ...
        public async Task ScoreUploadsWithDelayedScoreToken()
        {
            ...
            mockDatabase.Setup(db => db.GetScoreFromToken(2)).Returns(ValueTask.FromResult<SoloScore?>(new SoloScore
            {
                id = 3,
                passed = true
            }));
