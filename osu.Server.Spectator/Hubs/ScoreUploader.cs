...
namespace osu.Server.Spectator.Hubs
{
    public class ScoreUploader : IDisposable
    {
            ...
            try
            {
                ...
                using (var db = databaseFactory.GetInstance())
                {
                    while (queue.TryDequeue(out var item))
                    {
                        ...
                        {
                            await uploadScore(item.token, scoreId.Value, item.score);
                        }
                    }
                }
            }
        ...
        private async Task uploadScore(long token, long scoreId, Score score)
        {
            try
            {
            ...
            {
                // Never throw away replays, always re-queue.
                Enqueue(token, score);
                throw;
            }
        }
